...
class PotsdamDataModule(LightningDataModule):
    ...
    def test_dataloader(self) -> DataLoader:
        batch_size = self.cfg["data"]["batch_size"]["total_test"]
        n_workers = self.cfg["data"]["num_workers"]
...
class PotsdamDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if anno_mask.size(1) != img_height or anno_mask.size(2) != img_width:
            resize_transform = transforms.Resize((img_height, img_width), interpolation=InterpolationMode.BILINEAR)
            anno_mask = resize_transform(anno_mask)
