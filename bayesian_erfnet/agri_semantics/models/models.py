...
from semantic_segmentation.semseg.schedulers import WarmupPolyLR
...
class NetworkWrapper(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        elif self.cfg["train"]["scheduler"] == "warmuppolylr":
            scheduler_interval = "epoch"
            scheduler = WarmupPolyLR(
                optimizer,
                power=self.cfg["train"]["warmuppolylr"]["power"],
                max_iter=self.cfg["train"]["max_epoch"] * (int(self.num_train_data / min_batch_size) + 1),
                warmup_iter=self.cfg["train"]["warmuppolylr"]["warmup_iter"],
                warmup_ratio=self.cfg["train"]["warmuppolylr"]["warmup_ratio"],
                warmup='exp',
                last_epoch=-1
            )
        else:
...
class SegFormer(NetworkWrapper):
    def __init__(self, cfg: Dict, al_logger_name: str = "", al_iteration: int = 0, num_train_data: int = 1):
        super(SegFormer, self).__init__(
            cfg, al_logger_name=al_logger_name, al_iteration=al_iteration, num_train_data=num_train_data
        )

        self.model = modules.SegFormerModel(
            num_classes=cfg["model"]["num_classes"],
            backbone=cfg["model"]["backbone"],
        )
        print('Initialization of SegFormer')

    def replace_output_layer(self):
        self.model.decode_head.linear_pred = nn.Conv2d(self.model.decode_head.linear_pred.in_channels, self.cfg["model"]["num_classes"], kernel_size=1)
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> Dict:
        out, _ = self.forward(batch["data"])
        annotations = self.get_annotations(batch, train_stage=False)
        loss = self.get_loss(out, annotations)
        self.log("Validation/Loss", loss, prog_bar=True)
